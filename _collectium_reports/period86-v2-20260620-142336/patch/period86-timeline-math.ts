...
export function getPeriod86TimelinePosition(
  ...
  if (startYear === null || maxYear <= minYear) {
    return { isVisible: false, visibleStartYear: minYear, visibleEndYear: minYear, leftPct: 0, widthPct: 0 };
  }
  ...
  const widthPct = Math.max(0.4, ((visibleEndYear - visibleStartYear) / range) * 100);

  return { isVisible: true, visibleStartYear, visibleEndYear, leftPct, widthPct };
}

export function formatPeriod86YearRange(fromYear: number | string, toYear: number | string): string {
