 ...
 *
 * Overskrift:
 * Catalog Results API for Periodefilter
 *
 * Definering / formål:
 * Neon-basert API-rute som returnerer strukturerte katalogobjekter og deres relasjoner
 * for visning i de tre visningskortene (horisontal, liste, museum) på /test/periodefilter.
 *
 * Bruksområde:
 * Brukes av components/period-filter/CollectiumPeriodFilterClient.tsx for å hente katalogtreff.
 *
 * Berørte API-ruter:
 ...
 * - public.ct_v_catalog_period_relations
 *
 * Dataretning:
    ...
    return NextResponse.json(
      {
        ...
        source: "neon",
        route: "/api/catalog/results",
        message: error instanceof Error ? error.message : "Ukjent feil i results-api.",
        count: 0,
        objects: [],
      },
      { status: 500 }
